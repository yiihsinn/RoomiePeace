...
> 等一下大家可以看 Guided Demo 右邊的 trace：每一步都會顯示 router 判斷出的 intent、選到哪個 skill、呼叫哪些 tools、更新哪些 memory。  
...
> RoomiePeace 的核心不是聊天，而是 skill-based agent。它會 route、會 call tools、會更新 memory，也會用 guardrail 控制笑話的邊界。  
